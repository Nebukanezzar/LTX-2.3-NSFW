...
if [[ -x "$COMFYUI_DIR/.venv-cu128/bin/python" ]]; then
  PYTHON_BIN="$COMFYUI_DIR/.venv-cu128/bin/python"
...
while IFS=$'\t' read -r dirname repo; do
  ...
  # Skip installers that are unnecessary or risky on this RunPod image.
  if [[ "$dirname" == "ComfyUI-Impact-Pack" ]]; then
    log "Skipping Impact Pack install.py"
    continue
  fi

  if [[ "$dirname" == "ComfyUI-Frame-Interpolation" ]]; then
    log "Skipping Frame Interpolation install.py; requirements are already handled"
    continue
  fi
