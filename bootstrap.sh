...
# ---------------------------------------------------------------------------
# Impact Pack / SAM2 compatibility repair
#
# ComfyUI_LayerStyle_Advance includes its own top-level "sam2" package. That
# package can shadow the official Facebook SAM2 package and cause Impact Pack
# to fail with:
#   ImportError: attempted relative import beyond top-level package
#
# Keep the bundled copy as a backup, install the official SAM2 package without
# changing the template's pinned CUDA/PyTorch build, and then run Impact Pack's
# dependency/model checker.
# ---------------------------------------------------------------------------
IMPACT_PACK_DIR="$COMFYUI_DIR/custom_nodes/ComfyUI-Impact-Pack"
LAYERSTYLE_ADVANCE_DIR="$COMFYUI_DIR/custom_nodes/ComfyUI_LayerStyle_Advance"
LAYERSTYLE_SAM2_DIR="$LAYERSTYLE_ADVANCE_DIR/py/sam2"
LAYERSTYLE_SAM2_BACKUP="$LAYERSTYLE_ADVANCE_DIR/py/sam2_layerstyle_backup"

if [[ -d "$LAYERSTYLE_SAM2_DIR" ]]; then
  log "Moving LayerStyle Advance's conflicting SAM2 package out of Python's import path"
  rm -rf "$LAYERSTYLE_SAM2_BACKUP"
  mv "$LAYERSTYLE_SAM2_DIR" "$LAYERSTYLE_SAM2_BACKUP"
fi

log "Installing official SAM2 without replacing the existing PyTorch/CUDA build"
"$PYTHON_BIN" -m pip install \
  --disable-pip-version-check \
  --no-build-isolation \
  --no-deps \
...
if [[ -f "$IMPACT_PACK_DIR/install.py" ]]; then
  log "Running Impact Pack dependency and model checks"
  (
    cd "$IMPACT_PACK_DIR"
    COMFYUI_PATH="$COMFYUI_DIR" \
    COMFYUI_MODEL_PATH="$COMFYUI_DIR/models" \
      "$PYTHON_BIN" install.py
  )
else
  die "Impact Pack install.py was not found at: $IMPACT_PACK_DIR/install.py"
fi

log "Validating official SAM2 import"
(
  cd "$COMFYUI_DIR"
  "$PYTHON_BIN" - <<'PY'
import sam2
